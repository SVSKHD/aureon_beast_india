...
from aureon_mcx.market.aggregation import TimeframeAggregator, aggregate_closed
...
def test_m1_builder_closed_candles_only():
    ...
    assert b.add_tick(Tick("428291", 70000, t0, last_qty=2)) is None
    assert b.add_tick(Tick("428291", 70010, t0 + timedelta(seconds=20), last_qty=3)) is None
    assert b.partial is not None and b.partial.is_closed is False
    m1 = b.add_tick(Tick("428291", 69990, t0 + timedelta(seconds=61), last_qty=1))
    assert m1 is not None and m1.is_closed
    ...
    assert b.add_tick(Tick("428291", 1.0, t0 + timedelta(seconds=30), last_qty=1)) is None
    assert b.partial.low == 69990
    # wall clock flush
    assert b.flush_at(t0 + timedelta(seconds=90)) is None
    flushed = b.flush_at(t0 + timedelta(seconds=120))
    assert flushed is not None and flushed.close == 69990
...
def test_m1_builder_day_volume_delta():
    ...
    c = b.add_tick(Tick("428291", 1, t0 + timedelta(seconds=70), day_volume=135))
    assert c.volume == 30
...
def test_aggregation_no_incomplete_bar_leakage():
    ...
    first = emitted[0][1]
    assert first.timeframe is Timeframe.M15 and first.is_closed
    assert first.open == m5[0].open and first.close == m5[2].close
    assert first.high == max(c.high for c in m5[:3]) and first.low == min(c.low for c in m5[:3])
    assert first.volume == sum(c.volume for c in m5[:3])
    assert first.open_time == m5[0].open_time

    # partial bucket is never emitted
    agg2 = TimeframeAggregator(Timeframe.M5, Timeframe.M15, "GOLD", "428291")
    assert agg2.add(m5[0]) is None and agg2.add(m5[1]) is None
    assert agg2.partial is not None and agg2.partial.is_closed is False
    assert aggregate_closed(m5[:8], Timeframe.M15) == [e for _, e in emitted[:2]]
    # flush_at only emits after the boundary passes
    assert agg2.flush_at(m5[2].open_time) is None
    assert agg2.flush_at(m5[3].open_time) is not None


def test_h1_and_h4_alignment_in_ist():
    ...
    assert floor_to(ts, 14400).astimezone(IST).strftime("%H:%M") == "08:00"
    ...
    h4 = aggregate_closed(h1, Timeframe.H4)
    assert h4 == []  # 08:00-12:00 bucket is incomplete: nothing leaks
